...
Built upon work from chapter 13

Demonstates the use of librarys by moving classes into seperate files and using inheritence to show
an understading on how to make them work together with the main program
"""
...
# Player class that stores all the fundemental functions and properties for the player
...
for i in range(50):
    # Create an instance of the good block (see good block library)
    block = GoodBlock("coin.png")
...
for i in range(50):
    # create an instance of bad block (see bad block class)
    block = BadBlock("goomba.png")
...
while not done:
    ...
    # Calls the update function for the BadBlock and GoodBlock to allow the blocks to move
